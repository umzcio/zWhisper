...
private struct ShareCardView: View {
    ...
    private var hoursText: String { String(format: "%.1f", monthHours) }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                Button {
                    SettingsBlip.play(soundStyle)
                    onToast("Share card export coming soon")
                } label: {
    ...
    private var cardPreview: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            Spacer(minLength: 0)
            Text("\(hoursText) hours saved")
                .font(.system(size: 22, weight: .bold))
                .tracking(-0.44) // −0.02em at 22px
                .foregroundStyle(.white)
            Text("with zWhisper this month")
                .font(.system(size: 11))
                .foregroundStyle(ZWColor.text2)
            Spacer(minLength: 0)
            Text("\(totalWords.formatted(.number)) words · \(dictationWPM) wpm")
        ...
        .background {
            ZStack {
                LinearGradient(
                    stops: [
                        .init(color: Color(hex: "#1B1B26"), location: 0),
                        .init(color: Color(hex: "#0B0B0F"), location: 0.55),
                        .init(color: Color(hex: "#141024"), location: 1),
                    ],
                ...
                Circle()
                    .fill(RadialGradient(
                        colors: [ZWColor.accentPurple.opacity(0.45), .clear],
                        center: .center,
                        startRadius: 2,
                        endRadius: 64
                    ))
                ...
                Circle()
                    .fill(RadialGradient(
                        colors: [ZWColor.accentTeal.opacity(0.35), .clear],
                        center: .center,
                        startRadius: 2,
                        endRadius: 64
                    ))
    ...
    private func copyText() {
        ...
        NSPasteboard.general.setString(
            "I've saved \(hoursText) hours this month with zWhisper — on-device dictation",
            forType: .string
        )
